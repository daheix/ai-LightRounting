...
1. **Spec 批准后立即执行**：用户批准 spec 后，无需再次确认，直接进入实施阶段
2. **Plan 制定后立即执行**：制定完计划后，无需咨询同意，直接添加所有任务到 todos 并一次性完成整个方案
3. **任务无人工干预**：所有任务尽力无人工干预完成，失败时告警而非等待
4. **禁止中途暂停询问**：执行过程中禁止中途询问"是否继续"，失败时给出告警并尝试替代方案

### 25.3 禁止行为

- **禁止** 在 spec 批准后再次询问"是否开始实施"
- **禁止** 在 plan 制定后询问"是否添加到 todos"
- **禁止** 在任务执行中途询问"是否继续下一个任务"
- **禁止** 因害怕出错而过度确认，应直接执行并在失败时修复
