...
- [ ] **F1**: spec.md 存在且包含 5 个 ADDED Requirements（8 阶段框架/标准化任务结构/关键路径依赖/网络资源核查/验收标准）
- [ ] **F2**: tasks.md 存在且包含 8 Sprint × 42 任务，每任务含算法文档引用 + 代码路径 + 验收标准 + 依赖关系
- [ ] **F3**: checklist.md 存在且包含 12 通用检查点 + 8 Sprint 专项检查点 + 框架完整性检查点
- [ ] **F4**: 网络资源核查已完成（5 项关键资源：arXiv:2507.22301/LiDAR ISPD 2025/Apollo arXiv:2504.18813/FDTDX 0.6.2/PPO OES 2026）
- [ ] **F5**: 43 聚类算法文档均被引用（A01-J02），路径 `2026-2028开发计划/功能清单与实现/`
- [ ] **F6**: 关键路径依赖明确（A04→A09→F01 / A01+A02→C03 / D01+D03+D04→D05）
- [ ] **F7**: 8 个里程碑（M1-M8）对应 Sprint 验收点
- [ ] **F8**: 规则对齐（规则 1/7/10/14/17/18/22/25/26 + python代码开发规则.md）
