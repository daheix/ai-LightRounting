...
| 复刻包 | 原工具 | 用途 | 复刻原因 |
|--------|--------|------|----------|
| pyCopySiPANN | SiPANN | 硅光器件模型（10 个 S 参数模型复刻） | tensorflow 无 Python 3.14 wheel |

**注**：torch/sax/klayout 等活跃维护的工具直接用原工具 + 离线 wheel，不复刻（避免过度工程）。
...
项目已离线打包 torch 2.12.1+cpu（184MB），沙箱重启后 install.sh 自动恢复。

### Q: sax 依赖链过大怎么办？

A: sax 依赖 jax/jaxlib/optax（200+ MB），已全部离线打包。沙箱重启后 install.sh 自动恢复，
项目代码中直接 `import sax`，无回退。
