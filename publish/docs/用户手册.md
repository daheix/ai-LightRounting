...
约束检查器支持 9 种违规检查（弯曲半径/间距/损耗/交叉/重叠/最小宽度/耦合间隙/波导长度/层密度）：
...
PoLaRIS 内置 9 个 foundry DRC runset，覆盖 SOI/SiN/InP/LNOI 4 大材料平台：

```python
from polaris.sim.foundry_runsets import list_foundry_runsets, get_foundry_runset

# 列出所有 foundry runset
print(list_foundry_runsets())
# ['AMF', 'CompoundTek', 'GF_Fotonix', 'HHI_InP', 'IHP', 'LIGENTEC', 'LNOI', 'LioniX_InP', 'SiEPIC_EBeam']

# 使用特定 foundry 的 DRC 规则
runset = get_foundry_runset("SiEPIC_EBeam")
print(f"规则数: {len(runset.rules)}, 工艺: {runset.process_node}")
```
...
report = run_lvs(gds_path="out/layout.gds", circuit=circuit)
