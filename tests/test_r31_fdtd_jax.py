...
class TestYeeGrid3D:
    ...
    def test_cfl_timestep(self):
        ...
        dt = grid.cfl_timestep(eps_r_max=SOI_EPS_R_SI)
...
class TestGedneyPML:
    ...
    def test_pml_creation(self):
        ...
        grid.epsilon_r = jnp.ones((20, 20, 12)) * SOI_EPS_R_SI
        pml = GedneyPML(grid, n_layers=4, m=3)
        assert pml.n_layers == 4
        assert pml.m == 3
        # 通过 _build_sigma_profile 验证梯度
        sigma_x = pml._build_sigma_profile(20, 50e-9, "x")
        # sigma 在 PML 外边界最大，向内递减
        assert float(sigma_x[0]) > 0  # 外边界
        assert float(sigma_x[4]) == 0  # 非 PML 区域（第 n_layers 个点）
        # 梯度单调递减（从外到内）
        s0 = float(sigma_x[0])
        s1 = float(sigma_x[1])
        s2 = float(sigma_x[2])
    ...
    def test_pml_validation(self):
        ...
        grid.epsilon_r = jnp.ones((10, 10, 6)) * SOI_EPS_R_SI
    ...
    def test_damping_coefficients(self):
        ...
        # ca_x 形状为 (20, 1, 1)，取 [4,0,0] 为内边界
        assert float(ca_x[4, 0, 0]) == pytest.approx(1.0, abs=1e-10)
        # 外边界 Ca < 1
        assert float(ca_x[0, 0, 0]) < 1.0
    ...
    def test_pml_zero_layers(self):
        ...
        sigma_x = pml._build_sigma_profile(20, 50e-9, "x")
        assert float(jnp.sum(sigma_x)) == 0
...
class TestFDEModeSolver:
    ...
    def test_fde_solve_fundamental_neff(self):
        ...
        assert float(mode["neff"]) == pytest.approx(SOI_N_SI, rel=1e-3)
        # beta = 2*pi*neff/lambda
        expected_beta = 2 * math.pi * SOI_N_SI / 1.55e-6
        assert float(mode["beta"]) == pytest.approx(expected_beta, rel=1e-3)
        assert mode["Ex"].shape == (20, 20)
        # 高斯分布中心最强
        assert abs(float(mode["Ex"][10, 10])) >= abs(float(mode["Ex"][0, 0]))
...
class TestDifferentiableFDTD:
    ...
    *创新*: 基于 JAX jax.grad 自动微分，替代 lumopt 手动伴随方程。
    ...
    def test_fdtd_no_pml(self):
        """FDTD 无 PML 运行：仍能正常仿真。"""
        grid = YeeGrid3D(nx=16, ny=16, nz=8, dx=50e-9, dy=50e-9, dz=50e-9)
        fdtd = DifferentiableFDTD(grid, pml=None)
        eps_r = jnp.ones((16, 16, 8)) * SOI_EPS_R_SI
        result = fdtd.run(eps_r, (8, 8, 4), C0 / 1.55e-6, 30, (8, 8, 4))
        assert result["monitor_signal"].shape == (30,)
...
class TestJAXFDTDEngine:
    ...
    def test_engine_validation(self):
        ...
        with pytest.raises(ValueError, match="grid_size"):
            JAXFDTDEngine(grid_size=(10, 10))  # 非 3 元组
        with pytest.raises(ValueError, match="dx_um"):
            JAXFDTDEngine(dx_um=-1)
        with pytest.raises(ValueError, match="runtime_fs"):
            JAXFDTDEngine(runtime_fs=-1)
    ...
    def test_engine_setup_geometry(self):
        ...
        assert engine.fdtd is not None
    ...
    def test_engine_run_with_source(self):
        ...
        engine2.setup_geometry(np.ones((16, 16, 8)))
        with pytest.raises(RuntimeError, match="无源"):
            engine2.run()
...
class TestR31Integration:
    ...
    def test_pml_reflection(self):
        ...
        grid.epsilon_r = jnp.ones((40, 40, 20)) * SOI_EPS_R_SI
        pml = GedneyPML(grid, n_layers=8, m=3)
        dt = grid.cfl_timestep()
        ca_x, _, _, _, _, _ = pml.damping_coefficients(dt)
        # ca_x 形状为 (40, 1, 1)，取 [0,0,0] 外边界，[8,0,0] 内边界
        assert float(ca_x[0, 0, 0]) < 1.0  # 外边界存在阻尼
        assert float(ca_x[8, 0, 0]) == pytest.approx(1.0, abs=1e-10)  # 内边界无阻尼
        # 阻尼梯度：外边界 Ca < 内边界 Ca
        assert float(ca_x[0, 0, 0]) < float(ca_x[8, 0, 0])
