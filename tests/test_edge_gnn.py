...
def test_no_fallback():
    """测试 R03 无 fall-back（无 except:pass / return None 假数据）。"""
    import polaris.rl.edge_gnn as mod

    src = open(mod.__file__).read()
    # 禁止 except: pass / except: return None / return [] 假数据兜底
    assert "except:" not in src.replace("except:", "X", 0) or "except Exception" not in src
    # 无静默 except pass
    import re

    # 检查无 "except ...: pass" 或 "except ...: return None/[]"
    bad_patterns = [
        r"except[^:]*:\s*pass",
...
import re  # noqa: E402（测试用，放末尾避免影响主模块导入顺序）
