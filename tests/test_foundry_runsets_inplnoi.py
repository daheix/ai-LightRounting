...
def test_hhi_inp_runset_rules():
    """测试 HHI InP runset 规则数和阈值（第86轮新增 WG_DENSITY）。"""
    assert len(HHI_INP_DRC_RUNSET) == 8
...
def test_lionix_inp_runset_rules():
    """测试 LioniX InP runset 规则数和阈值（第86轮新增 WG_DENSITY）。"""
    assert len(LIONIX_INP_DRC_RUNSET) == 6
