...
| meep | —（未装） | GPLv2+ | GPL 传染：`import meep` 动态链接，分发时需 GPL 兼容。项目仅条件 import 用于器件级 FDTD 仿真，非核心功能 | ⚠️ 需编译 | modules/lumerical/src/polaris_lumerical/_backends.py（条件 import，check_meep_availability；`MeepAdjointBackend.run()` 第 319 行 raise NotImplementedError 未实现） | https://meep.readthedocs.io/en/stable/License_and_Copyright/ | 保留条件 import；商用版剥离 meep 依赖，改用自研 FDTD（ALGORITHMS.md#FDFD） |
