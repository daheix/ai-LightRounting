"""Clements 酉矩阵分解模块 — 通用 M×M 酉矩阵构造（玻色采样自带酉矩阵生成器）。
...
**Input**:
- ``n_modes``: 模式数 M（默认 4）。
- ``seed``: 随机种子（默认 42，决定分束器角度与相移，可复现）。
...
- Clements 网格交替层（偶数层从 0 开始，奇数层从 1 开始）左乘分束器，
  左乘酉保酉性，浮点误差 ~1e-15（无需 QR 修正）。

**Output**:
- M×M 酉矩阵，``list of list of [real, imag]``（实虚交错，与 C ABI
  row-major 实虚交错布局一致，可供 ``boson_sampling`` 直接使用）。

R03 合规: 酉性校验失败即 raise（不用 QR 兜底）。
