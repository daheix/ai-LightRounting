"""鲁棒性优化（蒙特卡洛制造公差扰动，worst-case / mean-case）。

从 v4 ``polaris/sim/robust_optimizer.py`` 迁移（R13 不保留 v4 兼容）。
...
- ``RobustObjective``: 鲁棒性目标函数（worst-case / mean-case / mean-std）
- ``RobustOptimizer``: 鲁棒性优化器（梯度下降 + 公差扰动）
- ``MonteCarloEvaluator``: 蒙特卡洛采样评估

来源（R02 学术诚信，≥5 文献 URL）:
- Wang et al. 2018 "Robust topology optimization of photonic devices",
  Opt. Express 26(18):23273, https://doi.org/10.1364/OE.26.023273
- Alexander et al. 2021 "Robust optimization of nanophotonic devices",
  Phys. Rev. Applied 16, 014013, https://doi.org/10.1103/PhysRevApplied.16.014013
- Tidy3D robust optimization:
  https://docs.flexcompute.com/projects/tidy3d/en/latest/
- lumopt robust optimization: https://lumopt.readthedocs.io/en/latest/
...
class ToleranceModel:
    ...
    def sample(
        self, params: np.ndarray, rng: np.random.Generator | None = None
    ) -> np.ndarray:
        """对参数采样制造公差扰动。"""
        if rng is None:
...
class MonteCarloEvaluator:
    ...
    来源: Wang et al. 2018, https://doi.org/10.1364/OE.26.023273
    """
...
class RobustObjective:
    ...
    来源: Alexander et al. 2021, https://doi.org/10.1103/PhysRevApplied.16.014013
    """

    def __init__(self, fom_fn: Callable[[np.ndarray], float], config: RobustConfig) -> None:
...
class RobustOptimizer:
    ...
    def optimize(
        ...
        for _ in range(self.config.max_iterations):
            robust_fom, _mean, _std, _worst = objective.evaluate(params)
    ...
    def _robust_gradient(
        self, params: np.ndarray, grad_fn: Callable[[np.ndarray], np.ndarray]
    ) -> np.ndarray:
    ...
    def _numerical_gradient(
        self, fom_fn: Callable[[np.ndarray], float]
...
def create_robust_optimizer(config: RobustConfig | None = None) -> RobustOptimizer:
...
def evaluate_robustness(
    ...
    evaluator = MonteCarloEvaluator(
        fom_fn=fom_fn, tolerance=tolerance, num_samples=num_samples, seed=seed
    )
