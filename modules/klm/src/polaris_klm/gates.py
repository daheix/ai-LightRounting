...
**Input**: 无（KLM CNOT 是固定电路，参数来自 Ralph 2002 PRA 65, 062324）。

**Process**:
- 构造 Ralph 2002 简化 4-BS CNOT 电路酉矩阵（4 模式:
  control=0 / target=1 / aux1=2 / aux2=3）:
...
- 分束器酉矩阵（KLM 约定，i 相位）:
      ...
      U_BS(θ) = [[cos θ,  i sin θ], [i sin θ, cos θ]]

- 验证电路酉性 U·U† = I（实算，误差 < 1e-10）。

**Output**:
- ``{success_prob: float, verified: bool}``
  - success_prob: 后选择成功率 = 1/9 ≈ 0.1111（Ralph 2002 理论值，表 I）。
  - verified: 电路酉性通过 + 报告值匹配理论值 1/9。
...
R03 合规: 电路酉性校验失败 raise，``success_prob`` 报告 Ralph 2002 理论值
（文献溯源，非硬编码 flag），``verified`` 通过实算校验。
