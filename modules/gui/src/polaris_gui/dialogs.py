"""GUI 对话框/IDE 组件 — 宏调试器 + 宏 IDE + 查看器守卫（dialogs.py）。

从 interactive.py 拆分而来（R11 质量门禁：文件≤800行），保留原始文献溯源。

实现 KLayout Macro IDE 风格的脚本调试与控制台能力，以及查看器只读模式守卫。

文献来源（R02 学术诚信，≥5 条）：
1. KLayout Scripting Manual https://www.klayout.org/doc-qt5/manual/scripting.html
2. Python bdb — Debugger framework https://docs.python.org/3/library/bdb.html
3. Python sys.settrace — 跟踪函数 https://docs.python.org/3/library/sys.html#sys.settrace
4. Python code.InteractiveConsole — 交互式控制台
   https://docs.python.org/3/library/code.html#code.InteractiveConsole
5. Siemens L-Edit Photonics（Viewer Mode 对标）
   https://eda.sw.siemens.com/en-US/ic/ic-custom/photonic/l-edit-photonics/
6. Gamma et al., "Design Patterns", Addison-Wesley 1994（MVC 分离）
   https://en.wikipedia.org/wiki/Command_pattern

*创新*：``MacroIDE`` 基于 ``sys.settrace``（bdb 底层机制）+
...
# === ViewerGuard 查看器只读模式守卫（对标 L-Edit Viewer Mode） ===
