"""GUI 版图编辑器交互组件 — 数据模型 + 命令模式 + 吸附 + 飞线（widgets.py）。

从 interactive.py 拆分而来（R11 质量门禁：文件≤800行），保留原始文献溯源。

实现 L-Edit / KLayout / OptoDesigner 风格交互式版图编辑器的核心数据组件：
- 曲线多边形（贝塞尔 / Catmull-Rom 样条 / 圆弧 / 椭圆 / 顶点编辑）
- 对象交互（snap-to-grid 抓取移动 / 拖放 / 飞线 airline）
- 命令模式（undo/redo 历史栈）
...
7. KLayout Rubber-band / airline https://www.klayout.de/doc-qt5/manual/rubberband.html
8. Python copy 模块（深拷贝） https://docs.python.org/3/library/copy.html
...
支持理论：MVC 分离（Gamma 1994）+ 命令模式（Gamma 1994）。
