"""PoLaRIS Web Server HTTP 路由层（routes.py）。
...
2. REST 设计规范: https://docs.python.org/3/library/http.server.html
3. KLayout DRC 文档: https://www.klayout.org/doc-qt5/manual/drc_runsets.html
4. KLayout LVS 文档: https://www.klayout.org/downloads/master/doc-qt5/manual/lvs_compare.html
5. OWASP Error Handling: https://owasp.org/www-community/Improper_Error_Handling
...
class PolarisHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_jobs_submit(self) -> None:
        """处理 POST /api/jobs: 提交作业（Recipe JSON 作为 body）。"""
        content_length = int(self.headers.get("Content-Length", 0))
        body = (
    ...
    def _handle_jobs_cancel(self, path: str) -> None:
        """处理 POST /api/jobs/{job_id}/cancel: 取消作业。"""
        job_id = path.split("/")[3]
        scheduler = _get_scheduler()
...
__all__ = ["PolarisHTTPRequestHandler"]
