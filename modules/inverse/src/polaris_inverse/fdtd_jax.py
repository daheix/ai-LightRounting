"""JAX 可微分 3D FDTD 求解器（polaris-inverse 子模块内核）。

迁移自 src/polaris/sim/fdtd_jax_backend.py 的核心三件套
（YeeGrid3D / GedneyPML / DifferentiableFDTD），保留逆向设计所需的
可微分时间步进内核，剥离 FDEModeSolver / SParamExtractor / JAXFDTDEngine
等高层封装（本子模块仅需 run + jax.grad 链路）。
