...
def _run_adjoint_build_solver():
    """构建 YeeGrid3D 网格 + DifferentiableFDTD 求解器（含 Gedney PML）。
    ...
    return grid, fdtd, source_pos, monitor_pos, source_freq, target_freq, nx, ny


def _run_adjoint_optimize_loop(
    n_iterations: int,
    learning_rate: float,
    width_param: jnp.ndarray,
    grad_fn,
    ...
    """heavy-ball 动量梯度上升优化循环，返回 fom_history 和 best checkpoint。

    来源: Polyak 1964 heavy-ball; Kingma & Ba 2014 Adam best-checkpoint 惯例
    """
    fom_history: list = []
    velocity = 0.0
    best_fom = -float("inf")
    ...
    for i in range(n_iterations):
        fom_val = float(
            fom_fn(width_param, fdtd, grid, source_pos, source_freq,
        ...
        grad_val = float(
            grad_fn(width_param, fdtd, grid, source_pos, source_freq,
                    FDTD_N_STEPS, monitor_pos, target_freq)
        )
    ...
    return fom_history, best_fom, best_width, width_param


def _run_adjoint_finalize(
    width_param: jnp.ndarray,
    fom_history: list,
    best_fom: float,
    best_width: float,
    ...
    """计算最终 FoM，组装并返回优化结果 dict（best-checkpoint 语义）。"""
    fom_final = float(
        fom_fn(width_param, fdtd, grid, source_pos, source_freq,
               FDTD_N_STEPS, monitor_pos, target_freq)
    )
