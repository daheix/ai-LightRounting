...
def _optimize_yield_validate(
    base_params: np.ndarray, param_sigmas: np.ndarray,
    direction: str, max_iter: int, learning_rate: float,
) -> tuple[np.ndarray, np.ndarray]:
    """校验 optimize_yield_via_nominal_shift 输入（R03 禁止 fall-back）。

    Returns:
        (base_params_copy, param_sigmas) 转换为 float ndarray。
    """
...
def _optimize_yield_iterate(
    ...
    每轮: 计算灵敏度 → σ_f → 梯度 → 试探步 → 评估新 WCD → 接受/终止。
    σ_f=0 raise（R03）；WCD 改善 < tol 收敛停止。

    Returns:
        (x_optimal, wcd_history, n_eval_total, converged, actual_iter)
    """
    converged = False
    actual_iter = 0
...
def optimize_yield_via_nominal_shift(
    ...
    - Madkour et al. 2015, DOI: 10.1109/TCSI.2015.2495251
    """
    base_params, param_sigmas = _optimize_yield_validate(
    ...
    x = base_params.copy()
    x, wcd_history, n_eval_total, converged, actual_iter = (
        _optimize_yield_iterate(
            func, x, param_sigmas, spec_threshold, direction, sign,
            max_iter, learning_rate, tol, sensitivity_delta,
            wcd_history, n_eval_total,
        )
    )
