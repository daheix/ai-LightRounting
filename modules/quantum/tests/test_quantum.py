"""polaris-quantum 子模块测试。

测试覆盖:
- test_boson_sampling: 4 模式 2 光子，prob_sum≈1.0（误差<1e-6）
- test_klm_cnot: success_prob=1/9≈0.111, verified=True
- test_hom: dip_depth=1.0, verified=True
- test_clements_unitary: 4x4 酉性误差<1e-10
...
from polaris_quantum import (  # noqa: E402
    boson_sampling,
    clements_unitary,
    hom_interference,
    klm_cnot,
...
def test_boson_sampling():
    """4 模式 2 光子玻色采样: prob_sum≈1.0（误差<1e-6）。

    输入 [1,1,0,0]（2 光子 4 模式），Clements 酉矩阵。
    输出模式数 = C(n_photons + n_modes - 1, n_photons) = C(5,2) = 10。
    ...
    assert result["n_outputs"] == 10, \
        f"n_outputs 期望 10，实际 {result['n_outputs']}"
    # 所有概率非负
    assert all(p >= 0.0 for p in result["prob_distribution"])


def test_klm_cnot():
    """KLM CNOT: success_prob=1/9≈0.111, verified=True。

    Ralph 2002 简化 4-BS 电路后选择成功率理论值 1/9。
    """
    result = klm_cnot()
    assert "success_prob" in result
    assert "verified" in result
    assert abs(result["success_prob"] - 1.0 / 9.0) < 1e-6, \
        f"success_prob 期望 1/9≈{1.0 / 9.0}，实际 {result['success_prob']}"
