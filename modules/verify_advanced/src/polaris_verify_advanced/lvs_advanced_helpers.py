"""LVS 进阶几何辅助函数（从 v4 polaris.sim.lvs_advanced_helpers 迁移，纯 NumPy/KLayout）。

KLayout 采用延迟导入（lazy import）：模块级 import 不依赖 klayout，仅在调用
GDS 加载/Region 获取函数时才 import klayout.db。这使得子模块在无 klayout 环境
...
from ._layer_map import get_layer_tuple
from ._types import _find_layer_index


def _load_layout(gds_path: str | Path):
    ...
    """
    import klayout.db as db  # 延迟导入：仅在加载 GDS 时需要 klayout
    path = Path(gds_path)
    if not path.exists():
...
def _get_region(layout, cell, layer_name: str):
    """获取指定层的 Region。
    ...
    Raises:
        ...
        ImportError: klayout 未安装。
    """
    import klayout.db as db  # 延迟导入：仅在获取 Region 时需要 klayout
...
def _bbox_um(shape, dbu: float) -> tuple[float, float, float, float]:
    """取形状包围盒（μm）。"""
    box = shape.bbox()
