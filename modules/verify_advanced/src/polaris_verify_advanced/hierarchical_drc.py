"""层次化 DRC 引擎（从 v4 polaris.sim.hierarchical_drc 迁移，纯 NumPy）。
...
合规: R02 学术诚信 / R03 禁止 fall-back / R04 不参与 GPU（纯 NumPy）/ R13 不保留 v4 兼容。
"""
...
from .klayout_drc import DRCCheckType, DRCRule
...
class HierarchicalDRC:
    ...
    def _segments_straddle(d1: float, d2: float, d3: float, d4: float) -> bool:
        """叉积 straddling test: AB 与 CD 互相跨立。"""
