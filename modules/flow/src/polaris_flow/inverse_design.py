...
class GANInverseDesigner:
    ...
    def _discriminator_train_iteration(self, real, bs, rng, lam):
        """单次判别器训练迭代：D 前向+反向+梯度惩罚+Adam 更新。

        返回 (d_loss, gp_val)。来源: Gulrajani et al. 2017 NeurIPS, arXiv:1704.00028
        """
        ...
        fake_f = fake.reshape(bs, -1)
        hr = np.maximum(0, real @ self.D_W1 + self.D_b1)
        d_real = hr @ self.D_W2 + self.D_b2
        ...
        gp_f = gf @ self.D_W2.T * (hf > 0)
        gp_r = gr @ self.D_W2.T * (hr > 0)
        gW1 = fake_f.T @ gp_f + real.T @ gp_r
    ...
    def _generator_train_step(self, bs, rng):
        """生成器训练：g_loss = -mean(D(G(z)))，反向传播 + Adam 更新。"""
        z = rng.standard_normal((bs, self.config.latent_dim))
        hg = np.maximum(0, z @ self.G_W1 + self.G_b1)
        ...
        grad_fake = (grad_d @ self.D_W2.T * (hd > 0)) @ self.D_W1.T
        grad_pre = grad_fake * sig * (1 - sig)
        gG_W2 = hg.T @ grad_pre
    ...
    def train_step(self, real_shapes: list) -> dict:
        ...
        for _ in range(5):
            d_loss, gp_val = self._discriminator_train_iteration(real, bs, rng, lam)
            d_loss_sum += d_loss
            gp_sum += gp_val
        g_loss = self._generator_train_step(bs, rng)
