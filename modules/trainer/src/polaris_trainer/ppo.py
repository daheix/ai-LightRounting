"""PPO 智能体（polaris-trainer 核心训练算法）。

实现 actor-critic 网络 + PPO 更新（clip + GAE）+ 断点续训 + 指标记录。
纯 NumPy 自动微分实现（``polaris_trainer._nn``），R04: 不参与 GPU。

迁移自 PoLaRIS v4 ``src/polaris/trainer/ppo.py``（保持算法逻辑一致，
仅切换 ``polaris.nn`` → ``polaris_trainer._nn`` import，使子模块独立）。
...
3. Stable-Baselines3 PPO 实现 https://stable-baselines3.readthedocs.io/
4. CleanRL PPO 单文件实现 https://github.com/vwxyzjn/cleanrl
5. Loshchilov & Hutter, 2017, SGDR 余弦退火 https://arxiv.org/abs/1608.03983
6. Engstrom et al., 2020, Implementation Matters in PPO
   https://arxiv.org/abs/2005.12729
7. OpenAI Spinning Up 高斯策略 logprob
   https://spinningup.openai.com/en/latest/spinningup/rl_intro/rl_intro_part2.html

来源: 迁移自 PoLaRIS v4 ``src/polaris/trainer/ppo.py``。
"""
...
from polaris_trainer._nn import Adam, Linear, Module, ReLU, Sequential, Tensor
...
class ActorCritic(Module):
    ...
    ``action_mean``（策略均值）与 ``value_head``（价值）。
    """
...
@dataclass
class RolloutBuffer:
    """PPO rollout 缓冲区（即 ReplayBuffer / 经验回放缓冲）。"""
...
@dataclass
class Minibatch:
    """PPO 小批量数据（将 _process_minibatch 的参数打包）。"""
...
def compute_gae(
    ...
    Args:
        ...
        config: PPO 配置（提供 gamma 与 gae_lambda），None 时用默认值。
...
class PPOAgent:
    ...
    - 价值函数 clip
    - orthogonal 初始化（通过 ``_nn.Linear`` 默认）
    ...
    def __init__(
        ...
        # ac.parameters() 已含 action_log_std（Module 属性自动收集），无需重复添加
