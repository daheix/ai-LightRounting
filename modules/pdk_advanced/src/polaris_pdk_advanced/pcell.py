"""PCell 多视图参数化单元（从 v4 迁移，R11 路标：版图参数化代码驱动）。

对标 gdsfactory @gf.cell + IPKISS PCell 多视图架构。
所有错误必须 raise，禁止 fall-back（R03）。

=== Input / Process / Output 三段式文档 ===

Input:
- PCellMultiView(name, params): 多视图 PCell 实例
- @polaris_cell 装饰器: 自动缓存 + 参数校验 + 命名唯一性
- TransformMatrix: 仿射变换引擎（rotate/scale/translate/shear/bezier）
- ai_generate_pcell(description, platform): 自然语言 → PCell 代码

Process:
- 三视图架构：Layout（多边形+端口）/ Circuit（S参数模型）/ Netlist（实例+连接）
- 【创新】Observer Pattern 自动同步：修改 Layout 自动更新 Circuit/Netlist
- LRU 缓存（maxsize=1024），参数校验（类型检查），命名唯一性
- 【创新】贝塞尔曲线变换引擎（gdsfactory 仅支持仿射+欧拉弯曲）
- 【创新】AI 辅助 PCell 代码生成（模板匹配，非 LLM 调用）

Output:
- PCellMultiView: 多视图 PCell（含 layout_view/circuit_view/netlist_view）
- PCellCache: LRU 缓存管理器（hit_rate/size）
- TransformMatrix: 仿射变换矩阵（apply/compose/inverse/bezier_transform）
- ai_generate_pcell: PCell Python 代码字符串
...
- Farin, "Curves and Surfaces for CAGD", 2002（贝塞尔曲线 Bernstein 多项式）
...
from polaris_pdk_advanced._base import BoundingBox, Device, Direction, Port
...
class TransformMatrix:
    ...
    def rotate(self, angle_deg: float) -> TransformMatrix:
        ...
        return self.compose(TransformMatrix(a=cos_a, b=-sin_a, c=sin_a, d=cos_a))

    def scale(self, sx: float, sy: float | None = None) -> TransformMatrix:
        """缩放变换（返回新矩阵，sy 为 None 时等比缩放）。"""
        if sy is None:
            sy = sx
        return self.compose(TransformMatrix(a=sx, d=sy))

    def translate(self, tx: float, ty: float) -> TransformMatrix:
        """平移变换（返回新矩阵）。"""
        return self.compose(TransformMatrix(tx=tx, ty=ty))

    def shear(self, kx: float, ky: float = 0.0) -> TransformMatrix:
        """剪切变换（返回新矩阵）。"""
        return self.compose(TransformMatrix(b=kx, c=ky))
...
class LayoutView:
    ...
    def on_view_changed(self, source: str, event: dict[str, Any]) -> None:
        """响应其他视图变化（Observer Pattern）。Layout 视图为主视图，不主动同步。"""
        pass
...
def _resolve_hints(func: Callable) -> dict[str, Any]:
    ...
    forward reference 解析失败时返回空 dict（typing 官方文档化行为），
    但记录 logger.debug 避免静默吞异常（R03 边界改进）。
    """
    try:
...
def ai_generate_pcell(description: str, platform: str = "SOI") -> str:
    ...
    Raises:
        ValueError: 无法识别的器件类型（R03 不 fall-back）。
    """
