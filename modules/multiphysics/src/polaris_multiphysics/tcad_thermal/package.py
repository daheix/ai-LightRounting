...
批次 10-B 拆分说明（2026-07-01）:
    ...
    - polaris_multiphysics.tcad_thermal.tcad_model: DopingType/TCADDeviceSpec/TCADAwareModel
      （等离子体色散 / PN 结耗尽 / 调制器 Vπ / 探测器响应度）
    - polaris_multiphysics.tcad_thermal.solver: ThermalLayer/ThermalSolver2D
      （2D 稳态/瞬态热传导 FDM + Carslaw-Jaeger 热串扰矩阵）
    - polaris_multiphysics.tcad_thermal.packaging: PackageType/PackageSpec/PackageDesigner
      （热预算 / 耦合损耗估算 / IO 计数）
    - polaris_multiphysics.tcad_thermal.testchip: TestType/TestStructure/TestChipDesigner
      （11 种标准测试结构 + 布图 + 测试计划）
    - polaris_multiphysics.tcad_thermal.m3: M3Deliverable（M3 里程碑交付物检查清单）
...
# 任何外部代码 `from polaris_multiphysics.tcad_thermal.package import X`
...
from polaris_multiphysics.tcad_thermal.solver import (
    ThermalLayer,
    ThermalSolver2D,
)
