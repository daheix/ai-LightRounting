...
提供稳定的 Python API（route_circuit/compute_path_loss/bend_compensate），
对已布局电路执行曲线波导布线，输出波导路径、总插入损耗、交叉数与弯曲数。
...
## 模块拆分（R11 质量门禁：文件 ≤800 行）

- ``__init__.py``（本文件）: route_circuit + compute_path_loss + 共享辅助函数
- ``route_compensate.py``: bend_compensate + 端口偏差补偿专用辅助函数
- ``curvy.py``: CurvyRouter / 曲线波导生成 / 损耗常数 / 几何工具
...
# bend_compensate 从 route_compensate.py 导入（R11 质量门禁：单文件 ≤800 行）
# 必须在所有共享辅助函数定义之后导入（route_compensate.py 依赖本模块的
# _validate_circuit/_validate_placements/_build_device_map/_find_port/
# _get_device_insertion_loss/_count_path_crossings）
from polaris_route.route_compensate import bend_compensate  # noqa: E402
