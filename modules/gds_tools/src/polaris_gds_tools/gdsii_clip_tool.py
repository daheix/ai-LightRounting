...
def clip_gdsii(
    ...
    ly = _clip_load_layout(db, in_path)
    dbu = float(ly.dbu)
    top_cell = _get_top_cell(ly, top_cell_name, gds_path)
    original_name = str(top_cell.name)
    shapes_before, bbox_before_um = _clip_compute_cell_stats(top_cell, ly, dbu)
    # 构造裁剪框（dbu 整数单位）
    box_dbu = db.Box(
        int(round(left / dbu)), int(round(bottom / dbu)),
        int(round(right / dbu)), int(round(top / dbu)),
    )
    # 层次化裁剪: Layout.clip(cell_index, box) -> new_cell_index
    # 来源: https://klayout.org/klayout-pypi/examples/clip/
    ...
    final_name = (
        clipped_cell_name if clipped_cell_name is not None else original_name
    )
    ...
    _clip_write_cell(new_cell, out_path)
    logger.info(
...
def multi_clip_gdsii(
    ...
    validated_boxes = _multi_clip_validate_inputs(in_path, clip_boxes_um)
    ...
    shapes_before, bbox_before_um = _clip_compute_cell_stats(top_cell, ly, dbu)
    # 构造 dbu 裁剪框列表
    box_dbu_list = _multi_clip_build_box_dbu_list(db, validated_boxes, dbu)
    # 多区域裁剪: Layout.multi_clip(cell_index, [box1, box2, ...]) -> [ci1, ci2, ...]
    ...
    for idx, (ci, box_um) in enumerate(zip(new_cis, validated_boxes)):
        reports.append(_multi_clip_process_one(
            idx, ci, box_um, ly, dbu, out_dir, prefix,
            original_name, gds_path, shapes_before, bbox_before_um,
        ))
    logger.info(
