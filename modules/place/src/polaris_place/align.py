...
矩阵型拓扑（Clements/Reck mesh 等）的检测与网格对齐已拆分到
``align_matrix.py``（保持 ``_detect_matrix_topology`` /
``_align_matrix_grid`` 通过本模块 ``__all__`` re-export 的 API 兼容）。
...
# 矩阵型拓扑检测/网格对齐已拆分到 align_matrix.py（R11 质量门禁）
from polaris_place.align_matrix import (
    _align_matrix_grid,
