...
从 v4 ``polaris.sim.parasitic_resistance`` 迁移至 polaris-parasitic 子模块
（R13: 不保留 v4 兼容路径）。纯 NumPy/SciPy CPU，R04 兼容。对齐 Synopsys
StarRC 电阻提取方法学。
...
class ParasiticResistor:
    ...
    def extract(
        ...
        Returns:
            {"resistance_ohm": float, "resistance_nominal_ohm": float,
             "n_squares": float, "temperature_c": float, "temp_factor": float}
