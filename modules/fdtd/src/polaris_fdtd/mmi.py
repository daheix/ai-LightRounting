...
def _simulate_mmi_setup_positions(
    nx: int, ny: int, nz: int, pml_layers: int,
) -> tuple[tuple[int, int, int], tuple[int, int, int], tuple[int, int, int]]:
    """计算源/双监视器位置（距 PML 至少 2 格，置于波导芯中心）。

    Returns:
        (source_pos, mon1_pos, mon2_pos)。
    """
    source_pos = (pml_layers + 2, ny // 2, nz // 2)
    # 双输出监视器（在输出波导芯中心）
    mon1_pos = (nx - pml_layers - 3, ny // 3, nz // 2)
    mon2_pos = (nx - pml_layers - 3, 2 * ny // 3, nz // 2)
    return source_pos, mon1_pos, mon2_pos


def _simulate_mmi_compute_stats(
    mon1: np.ndarray, mon2: np.ndarray, src_wave: np.ndarray,
) -> tuple[float, float, float]:
    """计算 MMI 双输出分束比/传输率/传输dB。

    Returns:
        (split_ratio, t_fdtd, transmission_db)。
    """
    # R03: NaN 校验
    ...
    return split_ratio, t_fdtd, transmission_db
...
def simulate_mmi_fdtd(
    ...
    """
    _simulate_mmi_validate(dx_um, n_steps, wavelength_um, nx, ny, nz, pml_layers)
    dx_m = dx_um * 1e-6
    ...
    source_pos, mon1_pos, mon2_pos = _simulate_mmi_setup_positions(
        nx, ny, nz, pml_layers,
    )
    t0 = time.time()
    ...
    split_ratio, t_fdtd, transmission_db = _simulate_mmi_compute_stats(
        mon1, mon2, src_wave,
    )
    return {
