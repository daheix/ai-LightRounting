...
# ---------------------------------------------------------------------------
# DeviceCatalog 序列化 Mixin（从 DeviceCatalog 拆分以控制类方法数，规则 4.2/5.3）
# ---------------------------------------------------------------------------
class _CatalogSerializationMixin:
    """DeviceCatalog 序列化 Mixin（JSON/YAML 导入导出）。

    将序列化方法集中至此 Mixin，使 ``DeviceCatalog`` 主体聚焦于注册与检索，
    符合单一职责原则（规则 5.3）。运行时 ``self`` 为 ``DeviceCatalog`` 实例，
    可访问 ``_devices`` / ``register`` 等属性。
    """
...
class DeviceCatalog(_CatalogSerializationMixin):
