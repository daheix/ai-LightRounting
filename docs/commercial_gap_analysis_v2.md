...
- 本修订所有数值均为 2026-07-17 在 v5.0 代码库实测：pytest 收集（2113 tests collected in 4.00s）、`list_platforms()` 调用、源码统计、文件系统核验，无估计值。
...
- **2026-07-17 核验修订**：子模块数从初版"34"修正为"33 pip 子模块"（`_c_abi` 为共享 C 头文件目录无 pyproject.toml，不计为 pip 子模块）；DRC 规则括号内"DEFAULT_DRC_RULES 18 条"修正为"全部位于 DEFAULT_DRC_RULES 列表中"（实测 25 条 DRCRule 实例全部在 DEFAULT_DRC_RULES 内，无游离实例）；功能清单中 GPUFDTDEngine 从"已移除"修正为"实验性"（该类实际存在于 `modules/lumerical/src/polaris_lumerical/_backends.py:200`，为纯 NumPy CPU 实现，类名保留 GPU 历史前缀）。
