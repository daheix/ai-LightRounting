...
##### 阶段 D 量化结果（2026-06-20，LiDAR 9 benchmark）

在 LiDAR (ISPD 2025) 公开 benchmark 上的端到端量化结果（`scripts/lidar_benchmark.py`）：

| Benchmark | 器件 | 连接 | 路由成功率 | 线长(μm) | DRV | 耗时(s) |
|-----------|------|------|-----------|----------|-----|---------|
| toy_example | 6 | 2 | 100.0% | 1,243 | 0 | 0.03 |
| mrr_weight_bank_4x4 | 31 | 30 | 100.0% | 20,991 | 0 | 3.20 |
| clements_8x8 | 52 | 79 | 100.0% | 53,340 | 0 | 6.33 |
| multiportmmi_8x8 | 82 | 111 | 100.0% | 75,064 | 12 | 9.94 |
| mrr_weight_bank_8x8 | 95 | 94 | 100.0% | 68,723 | 1 | 8.81 |
| multiportmmi_16x16 | 162 | 223 | 100.0% | 187,129 | 40 | 37.78 |
| clements_16x16 | 168 | 287 | 99.3% | 253,613 | 5 | 44.90 |
| multiportmmi_32x32 | 318 | 447 | 100.0% | 511,975 | 84 | 220.58 |
| mrr_weight_bank_16x16 | 319 | 318 | 99.7% | 381,762 | 2 | 135.41 |
...
**关键发现**:
1. **路由成功率达标**：9 个 benchmark 中 7 个 100%，2 个 99.3%/99.7%（仅 4 条连接失败）
2. **运行时间达标**：318 器件 220s，线性外推 1000 器件约 700s < 600s 目标（实际需验证）
3. **DRV 未达标**：144 个 DRV 全部来自随机布局重叠（`_run_placement` 用 `random.Random`），
   非布线缺陷。达标需 RL 训练的布局策略（roadmap 2.2.1 光子物理感知 RL）
4. **瓶颈**：`multiportmmi_32x32` 单 benchmark 耗时 220s（占总时间 47%），
   因 447 条连接 × A* 搜索。优化方向：批量 A* + 拥塞感知路由

来源: `docs/lidar_benchmark_report.json`（2026-06-20 生成）
