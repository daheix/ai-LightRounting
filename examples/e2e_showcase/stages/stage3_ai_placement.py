...
"""阶段 3: AI 布"""阶段 3: AI 布局。

使用 PoLaRIS v5.0 polaris-place 子模块对电路执行布局，
输出布局坐标与 HPWL（半周"""阶段 3: AI 布局。

使用 PoLaRIS v5.0 polaris-place 子模块对电路执行布局，
输出布局坐标与 HPWL（半周长线长）指标。

对应路标: R33（Edge-GNN 状态编码）/"""阶段 3: AI 布局。
...
PoLaRIS v5.0 迁移说明:
    旧 v4 在本 stage 自实现 PPO ActorCritic +"""阶段 3: AI 布局。
...
PoLaRIS v5.0 迁移说明:
    旧 v4 在本 stage 自实现 PPO ActorCritic + Edge-GNN 前向推理（~900 行）。
    v5.0 已将布局能力封装为 polaris-place 子模块的稳定 API ``place_circuit``，
    支持 ``"analytical"``（DREAMPlace 风格解析法布局，默认稳定）"""阶段 3: AI 布局。
...
PoLaRIS v5.0 迁移说明:
    ...
    ``"ppo_gnn"``（AlphaChip Edge-GNN + PPO，"""阶段 3: AI 布局。
...
PoLaRIS v5.0 迁移说明:
    ...
    本 stage 改用 ``place_circuit`` 子"""阶段 3: AI 布局。
...
PoLaRIS v5.0 迁移说明:
    ...
    支持 ``"analytical"``（DREAMPlace 风格解析法布局，默认稳定）与
    ``"ppo_gnn"``（AlphaChip Edge-GNN + PPO，需预训练 checkpoint）两种模式。
    本 stage 改用 ``place_circuit`` 子模块 API，删除全部自实现 PPO/G"""阶段 3: AI 布局。
...
布局算法来源:
- DREAMPlace DAC 2019: https://cseweb.ucsd.edu/classes/fa23/cse248-a/papers/placement/dreamplace.pdf
- DREAMPlace TCAD 2020: https://arxiv.org/abs/2004."""阶段 3: AI 布局。
...
- AlphaChip: Mirhoseini et al., Nature 2021
  https://www.nature.com/articles/s41586-021-03544-w
- TILOS MacroPlacement: https://github.com"""阶段 3: AI 布局。
...
from __future__ import annotations"""阶段 3: AI 布局。
...
from polaris_place import place"""阶段 3: AI 布局。
...
def _mzi_circuit() -> CircuitSpec:
    ...
    5 器件：1 光栅耦合器 + 2 MMI + 2 波导臂，构成马"""阶段 3: AI 布局。
...
def _clements_4x4_circuit() -> CircuitSpec:
    ...
    来源:
    - Clements et al., "Optimal design for universal"""阶段 3: AI 布局。
...
def _clements_4x4_circuit() -> CircuitSpec:
    ...
    来源:
    - Clements et al., "Optimal design for universal multiport interferometers",
      Optica 2016, https://doi.org/10.1364/OPTICA.3.001460"""阶段 3: AI 布局。
...
def _clements_4x4_circuit() -> CircuitSpec:
    ...
    devices: list[DeviceSpec] = []
    for i in range(6):
        devices.append(DeviceSpec(f"bs{i + 1}", "mm"""阶段 3: AI 布局。
...
def _clements_4x4_circuit() -> CircuitSpec:
    ...
    for i in range(6):
        devices.append(DeviceSpec(f"bs{i + 1}", "mmi_2x2", 20, 10))
    for i in range(4):
        devices.append(DeviceSpec(f"ps{i +"""阶段 3: AI 布局。
...
def _quantum_placeholder_circuit() -> CircuitSpec:
    ...
    return CircuitSpec(
        ...
        canvas_h=300,
        devices=[
            DeviceSpec("src1", "grating_coupler","""阶段 3: AI 布局。
...
def _quantum_placeholder_circuit() -> CircuitSpec:
    ...
    return CircuitSpec(
        ...
        devices=[
            ...
            DeviceSpec("src2", "grating_coupler", 10, 10),
            DeviceSpec("bs1", "mmi_2x2", 20, 10),
            DeviceSpec("det1", "detector", 10, 10"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    使用 ``place_circuit`` 解析法"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    来源:
    - DREAMPlace DAC 2019
      https://cseweb.ucsd.edu/classes/fa"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    - Kahng & Lienig, IEEE TCAD 2009"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    Args:
        circuit: 电路规格（CircuitSpec，内部转为 dict 供子模块"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    Returns:
        电路布局结果 dict，含 name/n_devices/placements/hpwl/ascii_layout/
        placement_mode/check"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    Raises:
        RuntimeError: 布局失败（R03 禁止 fall"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    ascii_layout = render_ascii_layout(
        circuit_dict, placements, grid"""阶段 3: AI 布局。
...
def _place_one_circuit(circuit: CircuitSpec) -> dict:
    ...
    return {
        ...
        "placements": placements,
        "hpwl": round(hpwl, 2),
        "ascii_layout": ascii_layout"""阶段 3: AI 布局。
...
def run(output_dir: Path)"""阶段 3: AI 布局。
...
def run(output_dir: Path) -> dict:
    ...
    流程:
    1. 构造 3 个演示电路（MZI、Clements 4x4、量子占位）
    2. 对每个电路调用 polaris-place ``place_circuit``（DREAMPlace 风格解析"""阶段 3: AI 布局。
...
def run(output_dir: Path) -> dict:
    ...
    3. 计算 HPWL 指标，输出 ASCII 布局预"""阶段 3: AI 布局。
