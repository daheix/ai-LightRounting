...
- 1000 底层逻辑：时频域联合互验 + 1000 器件向量化 S 参数级联。
  支持理论：见模块学术依据。
...
class InterconnectBackend:
    ...
    *创新*：时频域联合互验 + 1000 器件向量化 S 参数级联。
    ...
    def _sparams_to_fft_axis(self, s21: np.ndarray) -> np.ndarray:
        ...
        相位符号约定转换: 物理 S 参数采用 e^{+jωt} 约定（S21=exp(+jβL)），
        而 numpy FFT/IFFT 采用 e^{-jωt} 信号处理约定，正延迟 τ 对应频域
        相位 exp(-2πj·f·τ)。因此对插值后 S21 取共轭，将物理相位 +2π·τ·f
        转换为 FFT 约定 -2π·τ·f，保证 IFFT 后冲激响应在正延迟 t=+τ 处。
        文献: Oppenheim §3 (LTI 卷积定理); Pozar §4.3 (S 参数相位约定)。
        ...
        # 取共轭: 物理 e^{+jωt} → FFT e^{-jωt} 约定转换（相位符号取负）
    ...
    def analyze_eye_diagram(
        ...
        center = spb // 2
        levels = eye[:, center]
        # 用 25/75 百分位阈值分类 high/low，对 bit 不平衡鲁棒
        # （median 在不平衡 bits 下偏向多数类导致分类失败，
        #   百分位法对 NRZ 双电平信号始终能正确分群）
        p25, p75 = np.percentile(levels, [25, 75])
        if p75 <= p25:
            raise ValueError(
                "眼图电平分布退化（p25≈p75，单电平），无法分析"
            )
        threshold = (p25 + p75) / 2.0
        high = levels[levels > threshold]
        low = levels[levels <= threshold]
        ...
        # RMS 抖动: 交叉点时间方差
        crossings = self._find_crossings(eye, threshold)
    ...
    def _expand_one_subcircuit(
        ...
        支持两种嵌套形式:
        1. 子电路含 ``instances`` 字段: 遍历实例，若实例自身含
           ``subcircuits`` 则递归展开，否则作为普通器件加入。
        2. 子电路仅含 ``subcircuits`` 字段（无 ``instances``）:
           视为纯容器，递归展开其嵌套子电路。

        Raises:
            ValueError: 递归深度超限（防无限递归）或字段缺失。
        """
        if depth > 8:
        ...
        elif "subcircuits" in sub and sub["subcircuits"]:
            for nested in sub["subcircuits"]:
                self._expand_one_subcircuit(nested, comps, conns, depth + 1)
        else:
            raise ValueError(
                f"子电路 {sub.get('id')} 缺少 instances/subcircuits 字段"
            )
