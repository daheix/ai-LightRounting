...
*创新*: JAX autograd + 伴随方法共生 + 密度法二值化
  ...
  - 等价性: Hughes 2018 证 autograd = adjoint，无需手工推导伴随方程，实现成本更低。
...
# 启用 float64：伴随优化梯度精度要求高（Tidy3D/lumopt 均用 float64），
# float32 下有限差分验证梯度时精度不足（R05 修复）
...
# 文献 URL 常量（R02 学术诚信）
URL_TIDY3D = (
    "https://docs.flexcompute.com/projects/tidy3d/en/latest/notebooks/AdjointPlugin.html"
)
URL_LUMOPT = "https://github.com/pcrost/lumopt"
URL_MOLESKY = "https://arxiv.org/abs/1809.07731"
URL_PIGGOTT = "https://www.nature.com/articles/nphoton.2017.102"
URL_HUGHES = "https://arxiv.org/abs/1811.01255"
URL_WANG_FILTER = "https://doi.org/10.1007/s00158-004-0512-9"
...
class ModeOverlapObjective:
    ...
    def __init__(
        ...
        self.e_in = jnp.asarray(e_in, dtype=jnp.complex128)
        self.e_target = jnp.asarray(e_target, dtype=jnp.complex128)
    ...
    def _asm_step(self, field: jnp.ndarray, dz: float) -> jnp.ndarray:
        """角谱法传播单步（Goodman 1968 §3.10）。

        E(z+dz) = IFFT(FFT(E) · H(kx, dz))，H = exp(i·kz·dz)
        """
        k0 = 2.0 * jnp.pi / self.wavelength
        kx = 2.0 * jnp.pi * jnp.fft.fftfreq(field.shape[-1], d=self.dx)
        kz = jnp.sqrt((k0**2 - kx**2).astype(jnp.complex128))
        # 损耗波（kx²>k0²）指数衰减，物理正确
        transfer = jnp.exp(1j * kz * dz)
        return jnp.fft.ifft(jnp.fft.fft(field) * transfer)

    def _propagate(self, rho_proj: jnp.ndarray) -> jnp.ndarray:
        ...
        def step(carry: jnp.ndarray, rho_row: jnp.ndarray) -> tuple:
            modulated = carry * jnp.exp(1j * self.phi_max * rho_row)
            return self._asm_step(modulated, self.dz), None

        e_final, _ = jax.lax.scan(step, e_in, rho_proj)
        return e_final

    def overlap(self, e_out: jnp.ndarray) -> jnp.ndarray:
        """模式重叠积分 FOM = |⟨E_out, E_target⟩|² / (‖E_out‖²·‖E_target‖²)。"""
        num = jnp.abs(jnp.sum(e_out * jnp.conj(self.e_target))) ** 2
        den = jnp.sum(jnp.abs(e_out) ** 2) * jnp.sum(jnp.abs(self.e_target) ** 2)
        return num / den

    def forward(self, rho_proj: jnp.ndarray) -> jnp.ndarray:
        """正向仿真：ρ → 传播 → FOM（完全可微）。"""
...
class TopologyAdjointOptimizer:
    ...
    def _build_conic_kernel(self, radius_px: float) -> jnp.ndarray:
        ...
        kernel = jnp.maximum(0.0, 1.0 - r / radius_px)
        total = jnp.sum(kernel)
        if float(total) == 0.0:
            raise ValueError("锥形滤波核全零，filter_radius_px 过小")
        return kernel / total

    def set_design_region(self, shape: tuple[int, int]) -> None:
        """设置设计区域尺寸（重新初始化滤波核）。"""
        if len(shape) != 2 or shape[0] <= 0 or shape[1] <= 0:
            raise ValueError(f"shape 须为正二维元组，实际 {shape}")
        if shape[1] != int(self.objective.e_in.shape[0]):
            raise ValueError("shape[1] 须等于入口场维度")
        self.design_shape = shape
        self._filter_kernel = self._build_conic_kernel(self.filter_radius_px)
    ...
    def _drc_penalty_jax(self, rho_proj: jnp.ndarray) -> jnp.ndarray:
        ...
        """
        gy = jnp.gradient(rho_proj, axis=0)
        gx = jnp.gradient(rho_proj, axis=1)
        return jnp.mean(gx**2 + gy**2)

    def _total_objective(self, rho_raw: jnp.ndarray, beta: float) -> jnp.ndarray:
    ...
    def forward_simulate(self, design_vars: np.ndarray) -> dict:
        ...
        """
        rho = jnp.asarray(design_vars, dtype=jnp.float64)
    ...
    def compute_gradient(self, design_vars: np.ndarray, beta: float = 1.0) -> np.ndarray:
        ...
        *创新*: JAX autograd 与伴随方法数学等价（Hughes 2018），
        ...
        """
        rho_raw = jnp.asarray(design_vars, dtype=jnp.float64)
        # jax.grad 要求函数返回标量 jnp 数组，禁止 float() 包裹 tracer
        grad_fn = jax.grad(lambda r: self._total_objective(r, beta))
    ...
    def _beta_schedule(self, t: int) -> float:
        ...
        return self.config.beta_init + (self.config.beta_final - self.config.beta_init) * frac
    ...
    def optimize(self) -> TopologyOptimizationResult:
        ...
        """
        # 初始 ρ_raw = 0 → sigmoid(0)=0.5 中等密度（对称起点）
        ...
        for t in range(1, self.config.n_iters + 1):
            ...
            fom = float(self._total_objective(jnp.asarray(rho_raw), beta))
...
def example_grating_coupler() -> dict:
    """光栅耦合器优化示例：最大化向上衍射到高斯光纤模式。

    目标: 输入场经光栅耦合器向上衍射为高斯光纤模式。
    物理参数: λ=1.55μm，设计区 12×20 像素 @ 0.2μm。
    """
    h, w = 12, 20
    e_in = _gaussian_mode(w, sigma_px=1.0, center_px=(w - 1) / 2.0)
    e_target = _gaussian_mode(w, sigma_px=2.5, center_px=(w - 1) / 2.0)
    objective = ModeOverlapObjective(
        e_in.astype(np.complex64), e_target.astype(np.complex64)
    )
    config = OptimizerConfig(n_iters=40, learning_rate=0.05, drc_weight=0.005)
    ...
    return {
        ...
        "result": result,
        "coupling_efficiency": float(result.optimal_fom),
    }


def example_mode_converter() -> dict:
    """模式转换器优化示例：TE1 → TE0 模式转换。

    目标: 输入 TE1（双瓣反相）转换为 TE0（基模高斯）。
    物理参数: λ=1.55μm，设计区 10×18 像素 @ 0.2μm。
    """
    h, w = 10, 18
    # TE1 输入：双瓣反相（一阶厄米特-高斯近似）
    x = np.arange(w) - (w - 1) / 2.0
    e_in = x * np.exp(-(x**2) / (2 * 1.5**2))
    e_in = e_in.astype(np.complex64)
    norm = np.sqrt(np.sum(np.abs(e_in) ** 2))
    e_in = e_in / norm
    e_target = _gaussian_mode(w, sigma_px=1.5, center_px=(w - 1) / 2.0).astype(
        np.complex64
    )
    ...
    return {
        ...
        "result": result,
        "conversion_efficiency": float(result.optimal_fom),
    }
