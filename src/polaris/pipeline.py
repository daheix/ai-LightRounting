...
def _place_with_agent(env: FloorplanEnv, agent, n_devices: int) -> None:
    """使用 PPO 智能体进行布局放置。"""
...
def _place_random(env: FloorplanEnv, n_devices: int) -> None:
    """随机贪心布局（避免重叠的贪心放置）。"""
    for _ in range(n_devices):
        best = None
        best_reward = -1e9
...
def _place(net, devices, canvas: CanvasConfig, agent=None):
    ...
    if agent is not None:
        _place_with_agent(env, agent, len(devices))
    else:
        _place_random(env, len(devices))
...
def _save_report(net, devices, metrics, drc: DRCReport, out: Path) -> None:
    """保存运行报告 JSON。"""
    report = {
        "netlist": net.name,
...
def cmd_run(args):
    ...
    print("[3/5] 布线...")
    paths, congestion, metrics = _route(net, placements, canvas)
    print(f"      布线 {len(paths)} 连接, 总损耗 {metrics['total_loss_db']:.3f} dB")

    print("[4/5] 渲染与导出...")
    drc = _render_export_drc(placements, paths, congestion, net.name, out)

    _save_report(net, devices, metrics, drc, out)
