...
R317 实现:
- SIEPIC_EBEAM_SOI_RULESET: SiEPIC EBeam 220nm SOI 标准 DRC 规则集
- SIEPIC_EBEAM_SIN_RULESET: SiEPIC EBeam 300nm SiN 标准 DRC 规则集
- GENERIC_CONSERVATIVE_RULESET: 通用保守规则集
- get_preset_ruleset(name) -> list[CurvilinearDRCRule]: 按名获取预设规则集
- list_preset_rulesets() -> list[str]: 列出所有预设规则集名
- CustomRuleSetBuilder: 自定义规则集构建器（流式 API）
- validate_ruleset(rules) -> list[str]: 校验规则集完整性
...
R02 学术诚信:
- 所有规则参数附 SiEPIC PDK / Ligentec / 保守工艺文献 URL
- 规则集命名清晰，参数可溯源
...
- CurvilinearDRCRule: polaris.verification._drc_rules
...
# 参数依据: SiEPIC EBeam PDK klayout DRC 脚本
SIEPIC_EBEAM_SOI_RULESET: list[CurvilinearDRCRule] = [
    CurvilinearDRCRule(
        ...
        units="μm",
        description=(
            "SiEPIC EBeam 220nm SOI 波导最小宽度 0.35μm"
            "（SiEPIC EBeam PDK 标准）"
        ),
        severity="error",
    ),
    CurvilinearDRCRule(
        ...
        units="μm",
        description=(
            "SiEPIC EBeam 220nm SOI 波导最小间距 0.6μm"
            "（SiEPIC EBeam PDK 标准）"
        ),
        severity="error",
    ),
...
class CustomRuleSetBuilder:
    """自定义规则集构建器（R317，Builder Pattern）。
