    ...
    for li in ly.layer_indices():
        ...
        if polygon_count > 0:
            ...
            stats = LayerStats(
                ...
                total_area_um2=layer_area_um2,
                bbox_xmin=layer_xmin,
                bbox_ymin=layer_ymin,
                bbox_xmax=layer_xmax,
                bbox_ymax=layer_ymax,
            )
            layer_stats.append(stats)
            total_polygons += polygon_count
            total_area += layer_area_um2
            if layer_xmin < overall_xmin:
                overall_xmin = layer_xmin
            if layer_ymin < overall_ymin:
                overall_ymin = layer_ymin
            if layer_xmax > overall_xmax:
                overall_xmax = layer_xmax
            if layer_ymax > overall_ymax:
                overall_ymax = layer_ymax
