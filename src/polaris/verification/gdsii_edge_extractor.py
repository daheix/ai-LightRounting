...
def extract_edges(
    ...
    Raises:
        ...
        ValueError: 文件无效 / 层参数无效 / orientation_filter 无效 /
            output_path 提供但 layer_result 未提供 / top_cell_name 不存在 /
            层不存在 / max_samples <= 0。
    ...
    if min_length_um < 0:
        raise ValueError(
            f"min_length_um 不能为负: {min_length_um}。"
            f"禁止 fall-back（R03）。"
        )
    if max_length_um < 0:
        raise ValueError(
            f"max_length_um 不能为负: {max_length_um}。"
            f"禁止 fall-back（R03）。"
        )
    ...
    orientation_filter_norm = orientation_filter.upper() if orientation_filter else ""
    if orientation_filter_norm not in ("", "H", "V", "D"):
        raise ValueError(
    ...
    if not isinstance(max_samples, int) or max_samples <= 0:
        raise ValueError(
            f"max_samples 必须是正整数，得到: {max_samples}。"
            f"禁止 fall-back（R03）。"
        )
    ...
    for edge in edges.each():
        ...
        # 方向过滤
        if orientation_filter_norm and orientation != orientation_filter_norm:
            continue
    ...
    # 统计
    total_edges_after = len(edge_infos)
    if total_edges_after > 0:
        lengths = [e.length_um for e in edge_infos]
        total_length_um = sum(lengths)
        min_edge_length_um = min(lengths)
        max_edge_length_um = max(lengths)
        avg_edge_length_um = total_length_um / total_edges_after
        horizontal_count = sum(1 for e in edge_infos if e.orientation == "H")
        vertical_count = sum(1 for e in edge_infos if e.orientation == "V")
        diagonal_count = sum(1 for e in edge_infos if e.orientation == "D")
    else:
        total_length_um = 0.0
        min_edge_length_um = 0.0
        max_edge_length_um = 0.0
        avg_edge_length_um = 0.0
        horizontal_count = 0
        vertical_count = 0
        diagonal_count = 0

    length_histogram = _build_length_histogram(edge_infos)

    # 样本（前 max_samples 条）
    sample_edges = edge_infos[:max_samples]

    # 可选: 输出到新层
    if output_path is not None and layer_result is not None:
        li_tgt = ly.layer(tgt_layer[0], tgt_layer[1])
        # 把每条边作为细矩形（width=1 dbu）插入
        # 来源: GDSII 不支持 Edge，需转换为 polygon
        for e in edge_infos:
    ...
    logger.info(
        ...
        total_length_um, horizontal_count, vertical_count, diagonal_count,
    )
