...
def compile_all() -> int:
    ...
    print(f"[Cython] 开始编译（{nthreads} 线程）...")
    ...
    print(f"[Cython] Cython 编译完成: {len(ext_modules)} 个扩展模块")

    # 构建 .so 文件（就地，在副本目录中）
    distribution = Distribution({"ext_modules": ext_modules})
    build_ext_cmd = distribution.get_command_obj("build_ext")
    build_ext_cmd.ensure_finalized()
    build_ext_cmd.inplace = 1  # 就地编译到副本目录
    build_ext_cmd.parallel = nthreads
    build_ext_cmd.run()

    # 验证 .so 文件已生成
    so_files = list(RELEASE_MODULES.rglob("*.so"))
